...
type ComplaintUrgency = 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
...
export class GrievanceContract extends Contract {
  ...
  // ================================
  private validateUrgency(value: any) {
  ...
  async ComplaintExists(ctx: Context, id: string): Promise<boolean> {
  ...
  // CREATE COMPLAINT
  // ================================
  async CreateComplaint(ctx: Context, complaintJSON: string): Promise<void> {
    const complaint: Complaint = JSON.parse(complaintJSON);

    // Basic validation
    if (!complaint.id) throw new Error("Missing complaint.id");
    if (!complaint.description) throw new Error("Missing description");
    if (!complaint.categoryId) throw new Error("Missing categoryId");
    if (!complaint.complainantId) throw new Error("Missing complainantId");
    ...
    if (exists) throw new Error(`Complaint ${complaint.id} already exists`);
  ...
  // READ
  // ================================
  async GetComplaint(ctx: Context, id: string): Promise<string> {
    const data = await ctx.stub.getState(id);
    if (!data || data.length === 0) throw new Error(`Complaint ${id} not found`);
  ...
  // UPDATE COMPLAINT
  // ================================
  async UpdateComplaint(ctx: Context, updatedJSON: string): Promise<void> {
    const incoming: Complaint = JSON.parse(updatedJSON);

    if (!incoming.id) throw new Error("Missing complaint.id");

    const data = await ctx.stub.getState(incoming.id);
    if (!data || data.length === 0) throw new Error(`Complaint ${incoming.id} not found`);

    const existing: Complaint = JSON.parse(data.toString());
  ...
  async UpdateStatus(
    ...
    if (!data || data.length === 0) throw new Error(`Complaint ${complaintId} not found`);

    const complaint: Complaint = JSON.parse(data.toString());
  ...
  // QUERY ALL
  // ================================
  async QueryAll(ctx: Context): Promise<string> {
  ...
  // QUERY BY USER
  // ================================
  async QueryByUser(ctx: Context, userId: string): Promise<string> {
    const query = {
  ...
  // QUERY BY STATUS
  // ================================
  async QueryByStatus(ctx: Context, status: ComplaintStatus): Promise<string> {
  ...
  // GET HISTORY
  // ================================
  async GetHistory(ctx: Context, id: string): Promise<string> {
    const iterator: any = await ctx.stub.getHistoryForKey(id);
    const history: any[] = [];
